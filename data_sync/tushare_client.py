...
class TushareClient:
    ...
    def get_holder_number(self, ts_code):
        ...
        try:
            ...
            if df.empty:
                logger.warning(f"未获取到 {ts_code} 股东户数数据")
                return df
            
            df = df.sort_values('end_date')
            logger.info(f"获取 {ts_code} 股东户数数据 {len(df)} 条")
            return df
        except Exception as e:
            logger.error(f"获取 {ts_code} 股东户数数据失败: {e}")
            return pd.DataFrame()

    def get_daily_basic(self, ts_code=None, trade_date=None, fields=''):
        """
        获取每日指标数据（PE/PB/PS/市值等）
        ...
        try:
            df = self.pro.daily_basic(
                ...
                fields=fields or 'ts_code,trade_date,close,turnover_rate,turnover_rate_f,volume_ratio,pe,pe_ttm,pb,ps,ps_ttm,dv_ratio,dv_ttm,total_share,float_share,free_share,total_mv,circ_mv'
            )
            ...
            df = df.sort_values('trade_date')
            logger.info(f"获取每日指标数据 {len(df)} 条")
            return df
        except Exception as e:
            logger.error(f"获取每日指标数据失败: {e}")
            return pd.DataFrame()

    def get_fina_indicator(self, ts_code=None, fields=''):
        """
        获取财务指标数据（ROE/ROA/利润率/增长率等）
        ...
        try:
            df = self.pro.fina_indicator(
                ts_code=ts_code,
                fields=fields or 'ts_code,end_date,ann_date,eps,bps,roe,roe_dt,roa,npta,eps_yoy,bps_yoy,roe_yoy,or_yoy,op_yoy,ebt_yoy,tr_yoy,op_yoy_f,ebt_yoy_f,tr_yoy_f,grossprofit_margin,netprofit_margin,debt_to_assets,current_ratio,quick_ratio,ar_turnover,inv_turnover'
            )
            ...
            df = df.sort_values('end_date')
            logger.info(f"获取财务指标数据 {len(df)} 条")
            return df
        except Exception as e:
            logger.error(f"获取财务指标数据失败: {e}")
            return pd.DataFrame()

    def get_income_data(self, ts_code=None, fields=''):
        """
        获取利润表数据
        ...
        try:
            df = self.pro.income(
                ts_code=ts_code,
                fields=fields or 'ts_code,end_date,ann_date,revenue,operate_cost,total_profit,n_income,n_income_attr_p,total_assets,total_liab,total_share,revenue_yoy,op_yoy_f'
            )
            ...
            df = df.sort_values('end_date')
            logger.info(f"获取利润表数据 {len(df)} 条")
            return df
        except Exception as e:
            logger.error(f"获取利润表数据失败: {e}")
            return pd.DataFrame()

    def get_money_flow(self, ts_code=None, trade_date=None, fields=''):
        """
        获取资金流向数据
        ...
        try:
            df = self.pro.moneyflow(
                ts_code=ts_code,
                trade_date=trade_date,
                fields=fields or 'ts_code,trade_date,buy_sm_amount,sell_sm_amount,buy_md_amount,sell_md_amount,buy_lg_amount,sell_lg_amount,buy_elg_amount,sell_elg_amount,net_mf_amount,net_mf_vol'
            )
            ...
            df = df.sort_values('trade_date')
            logger.info(f"获取资金流向数据 {len(df)} 条")
            return df
        except Exception as e:
